...
public class Main {
			...
			if (seedInitialization == false) {
				...
				if (executableAlgorithm.equals("GP")) {
					...
					if (seedMutatedInitialization == true) {
						...
						gp.population = getSeededPopulation(seedPopulation, gp.getPopulation());
					}
					bestFound = gp.evolve(NUMBER_OF_GENERATIONS);
				} else if (executableAlgorithm.equals("GSGP")) {
					...
					if (seedMutatedInitialization == true) {
						...
						gsgp.population = getSeededPopulation(seedPopulation, gsgp.getPopulation());
					}
					bestFound = gsgp.evolve(NUMBER_OF_GENERATIONS);
				}
			}
	...
	// Replaces the worst individuals from the gsgp population with the best
	...
	private static void saveDataToFile() throws IOException {
		...
		for (int i = 0; i < Main.NUMBER_OF_GENERATIONS; i++) {
			...
			if (i != Main.NUMBER_OF_GENERATIONS - 1) {
				bw.write("\n");
			}
		}
		bw.close();
